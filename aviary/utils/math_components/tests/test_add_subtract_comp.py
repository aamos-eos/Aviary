...
from atlas.utils import AddSubtractComp
...
class TestAddSubtractCompScalars(unittest.TestCase):
    def setUp(self):
        ...
        adder = self.p.model.add_subsystem(name="add_subtract_comp", subsys=AddSubtractComp())
...
class TestWrongScalingFactorCount(unittest.TestCase):
    def setUp(self):
        ...
        adder = self.p.model.add_subsystem(name="add_subtract_comp", subsys=AddSubtractComp())
        adder.add_equation(
            "adder_output", ["input_a", "input_b", "input_c"], vec_size=self.nn, length=3, scaling_factors=[1, -1]
        )
...
class TestForDocs(unittest.TestCase):
    def test(self):
        ...
        p.model.add_subsystem(name="ivc", subsys=ivc, promotes_outputs=["thrust", "drag", "lift", "weight"])
