...
from atlas.utils import Integrator
...
class IntegratorTestGroup(Group):
    ...
    def setup(self):
        ...
        integ = Integrator(diff_units=diff_units, num_nodes=num_nodes, method=integrator_option, time_setup=time_setup)
        if not test_auto_names:
            ...
            integ.add_integrand(
                "q", units=quantity_units, rate_units=rate_units, zero_start=zero_start, final_only=final_only
            )
        if second_integrand:
            ...
            iv.add_output("initial_value_2", val=0.0, units="kJ")
...
class IntegratorCommonTestCases(object):
    ...
    def test_quadratic_rate_units(self):
        ...
        prob = Problem(
            IntegratorTestGroup(num_nodes=self.num_nodes, integrator=self.integrator, rate_units="kg/s", diff_units="s")
        )
        prob.setup(check=True, force_alloc_complex=True)
        prob["iv.rate_to_integrate"] = fprime
        prob.run_model()
    ...
    def test_quadratic_no_rate_units(self):
        ...
        prob = Problem(IntegratorTestGroup(num_nodes=self.num_nodes, integrator=self.integrator, diff_units="s"))
        prob.setup(check=True, force_alloc_complex=True)
        prob["iv.rate_to_integrate"] = fprime
        prob.run_model()
...
class EdgeCaseTestCases(unittest.TestCase):
    ...
    def test_default_value_vector(self):
        ...
        # DO NOT RUN THE MODEL
        assert_near_equal(
            prob.get_val("integral.q", units="kg"), 5.0 * np.linspace(0.0, 1.0, num_nodes), tolerance=1e-14
        )
        assert_near_equal(prob.get_val("integral.q_final", units="kg"), 5.0, tolerance=1e-14)
        partials = prob.check_partials(method="cs", compact_print=True)
        assert_check_partials(partials, atol=1e-8, rtol=1e0)
