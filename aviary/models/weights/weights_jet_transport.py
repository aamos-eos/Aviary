...
class WingWeight_JetTransport(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        W_wing_Raymer = (
            ...
            * np.cos(inputs["ac|geom|wing|c4_sweep"]) ** -1
    ...
    def compute_partials(self, inputs, J):  # TO DO
        ...
        J["W_wing", "ac|geom|wing|c4_sweep"] = (
            0.0051
            * (inputs["ac|weights|MTOW"] * n_ult) ** 0.557
            ...
            * (1 + inputs["ac|geom|wing|taper"]) ** 0.1
            * -1
            * np.cos(inputs["ac|geom|wing|c4_sweep"]) ** -2
            * (-1 * np.sin(inputs["ac|geom|wing|c4_sweep"]))
            * (self.options["control_surface_area_frac"] ** 0.1)
        )
...
class HstabWeight_JetTransport(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("ac|geom|hstab|c4_sweep", units="rad", desc="Quarter-chord sweep angle")
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["W_hstab"] = (
            ...
            * c4_wing_c4_tail**-1
            * np.cos(inputs["ac|geom|hstab|c4_sweep"]) ** -1
    ...
    def compute_partials(self, inputs, J):
        ...
        J["W_hstab", "ac|geom|hstab|c4_sweep"] = (
            0.0379
            * K_uht
            * inputs["HstabConst"] ** -0.25
            * inputs["ac|weights|MTOW"] ** 0.639
            ...
            * (0.3 * c4_wing_c4_tail) ** 0.704
            * c4_wing_c4_tail**-1
            * np.cos(inputs["ac|geom|hstab|c4_sweep"]) ** -2
            * np.sin(inputs["ac|geom|hstab|c4_sweep"])
            * inputs["ac|geom|hstab|AR"] ** 0.166
            * (1 + Se_Sht) ** 0.1
        )
...
class VstabWeight_JetTransport(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("ac|geom|vstab|c4_sweep", units="rad", desc="Quarter-chord sweep angle")
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["W_vstab"] = (
            ...
            * np.cos(inputs["ac|geom|vstab|c4_sweep"]) ** -1
    ...
    def compute_partials(self, inputs, J):
        ...
        J["W_vstab", "ac|geom|vstab|c4_sweep"] = (
            0.0026
            * (1 + Ht_Hv) ** 0.225
            * inputs["ac|weights|MTOW"] ** 0.556
            * n_ult**0.536
            * c4_wing_c4_tail ** (-0.5 + 0.875)
            * inputs["ac|geom|vstab|S_ref"] ** 0.5
            * np.cos(inputs["ac|geom|vstab|c4_sweep"]) ** -2
            * np.sin(inputs["ac|geom|vstab|c4_sweep"])
            * inputs["ac|geom|vstab|AR"] ** 0.35
            * inputs["ac|geom|vstab|toverc"] ** -0.5
        )
...
class FuselageKws_JetTransport(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("ac|geom|wing|AR", desc="Wing aspect ratio")
        self.add_input("ac|geom|wing|c4_sweep", units="rad", desc="Wing aspect Ratio")
    ...
    def compute(self, inputs, outputs):
        Kws_raymer = (
            ...
            * inputs["ac|geom|wing|AR"] ** 0.5
            * np.tan(inputs["ac|geom|wing|c4_sweep"])
    ...
    def compute_partials(self, inputs, J):
        ...
        J["K_ws", "ac|geom|wing|c4_sweep"] = (
            0.75
            * (1 + 2 * inputs["ac|geom|wing|taper"])
            / (1 + inputs["ac|geom|wing|taper"])
            * inputs["ac|geom|wing|S_ref"] ** 0.5
            * inputs["ac|geom|wing|AR"] ** 0.5
            * (1 / np.cos(inputs["ac|geom|wing|c4_sweep"])) ** 2
            * inputs["ac|geom|fuselage|length"] ** -1
        )
