...
class WingWeight_SmallTurboprop(ExplicitComponent):
    """Inputs: MTOW, ac|geom|wing|S_ref, ac|geom|wing|AR, ac|geom|wing|c4_sweep, ac|geom|wing|taper, ac|geom|wing|toverc, V_H (max SL speed)
    Outputs: W_wing
    ...
    def setup(self):
        ...
        self.add_input("ac|geom|wing|c4_sweep", units="rad", desc="Quarter-chord sweep angle")
    ...
    def compute(self, inputs, outputs):
        ...
        # W_wing_USAF = 96.948*((inputs['ac|weights|MTOW']*n_ult/1e5)**0.65 * (inputs['ac|geom|wing|AR']/math.cos(inputs['ac|geom|wing|c4_sweep']))**0.57 * (inputs['ac|geom|wing|S_ref']/100)**0.61 * ((1+inputs['ac|geom|wing|taper'])/2/inputs['ac|geom|wing|toverc'])**0.36 * (1+inputs['V_H']/500)**0.5)**0.993
        ...
        # c2sweep_wing = inputs['ac|geom|wing|c4_sweep'] # a hack for now
        ...
        W_wing_Raymer = (
            ...
            * (inputs["ac|geom|wing|AR"] / math.cos(inputs["ac|geom|wing|c4_sweep"]) ** 2) ** 0.6
            * inputs["ac|q_cruise"] ** 0.006
            * inputs["ac|geom|wing|taper"] ** 0.04
            * (100 * inputs["ac|geom|wing|toverc"] / math.cos(inputs["ac|geom|wing|c4_sweep"])) ** -0.3
    ...
    def compute_partials(self, inputs, J):
        ...
        J["W_wing", "ac|geom|wing|AR"] = (
            ...
            * inputs["ac|weights|W_fuel_max"] ** 0.0035
            * 0.6
            * (inputs["ac|geom|wing|AR"] / math.cos(inputs["ac|geom|wing|c4_sweep"]) ** 2) ** (0.6 - 1)
            / math.cos(inputs["ac|geom|wing|c4_sweep"]) ** 2
            * inputs["ac|q_cruise"] ** 0.006
            * inputs["ac|geom|wing|taper"] ** 0.04
            * (100 * inputs["ac|geom|wing|toverc"] / math.cos(inputs["ac|geom|wing|c4_sweep"])) ** -0.3
            * (n_ult * inputs["ac|weights|MTOW"]) ** 0.49
        ...
        c4multa = (inputs["ac|geom|wing|AR"] / math.cos(inputs["ac|geom|wing|c4_sweep"]) ** 2) ** 0.6
        c4multb = (100 * inputs["ac|geom|wing|toverc"] / math.cos(inputs["ac|geom|wing|c4_sweep"])) ** -0.3
        dc4multa = (
            0.6
            * (inputs["ac|geom|wing|AR"] / math.cos(inputs["ac|geom|wing|c4_sweep"]) ** 2) ** (0.6 - 1)
            * (-2 * inputs["ac|geom|wing|AR"] / math.cos(inputs["ac|geom|wing|c4_sweep"]) ** 3)
            * (-math.sin(inputs["ac|geom|wing|c4_sweep"]))
        )
        dc4multb = (
            -0.3
            * (100 * inputs["ac|geom|wing|toverc"] / math.cos(inputs["ac|geom|wing|c4_sweep"])) ** (-0.3 - 1)
            * -100
            * inputs["ac|geom|wing|toverc"]
            / math.cos(inputs["ac|geom|wing|c4_sweep"]) ** 2
            * (-math.sin(inputs["ac|geom|wing|c4_sweep"]))
        )
        J["W_wing", "ac|geom|wing|c4_sweep"] = c4const * (c4multa * dc4multb + c4multb * dc4multa)
        ...
        J["W_wing", "ac|geom|wing|toverc"] = (
            ...
            * -0.3
            * (100 * inputs["ac|geom|wing|toverc"] / math.cos(inputs["ac|geom|wing|c4_sweep"])) ** (-0.3 - 1)
            * (100 / math.cos(inputs["ac|geom|wing|c4_sweep"]))
            * (n_ult * inputs["ac|weights|MTOW"]) ** 0.49
        )
...
if __name__ == "__main__":
    ...
    dvs.add_output("ac|geom|wing|c4_sweep", 1.0, units="deg")
