...
class PropellerData:
    ...
    def load_data(cls, prop_filename, sheet_name='data'):
        ...
        if cls.dyn_rpm_data__rpm is None:  # Only load if not already loaded
            ...
            input_cols = ['densAlt_ft','Ct','SHP', 'advJ', 'Cp']  # replace with your actual input columns
            ...
            # Drop values at low fficiency
            df = df[df['eta'] >0]

            # Drop High Advance Ratio Points
            df =df[df['advJ'] < 2.53]
            ...
            tip_mach_data = df['tipMach'].values
            
            # Convert to SI units
            disa_data__K = disa_data__degC + 273.15
            dens_alt_data__m = dens_alt_data__ft * 0.3048

            alt_data = dens_alt_data__m
            temp_degC = 15.04 - 0.00649 * alt_data
            press_kPa = 101.29 * ((temp_degC + 273.15) / 288.08) ** 5.256
            rho_kgpm3 = press_kPa / (0.2869 * (temp_degC + 273.15))

            eas_data__mps = eas_data__kts * 0.514444
            # Convert to true airspeed
            tas_data__mps = eas_data__mps * np.sqrt(1.225 / rho_kgpm3)
            ktas_data__kts = tas_data__mps / 0.514444
            ...
            cls.stat_tip_mach_data = tip_mach_data[stat_inds]
            ...
            cls.dyn_tip_mach_data = tip_mach_data[dyn_inds]
            
            #print(f"Static data points: {len(cls.stat_thrust_data__N)}")
            #print(f"Dynamic data points: {len(cls.dyn_thrust_data__N)}")
    
    @classmethod
    def load_rpm_schedule(cls, prop_filename, sheet_name='RPM_schedule'):
        """Load RPM schedule data once and store it in the class variables"""
        if cls.rpm_sched_data__rpm is None:
            df_rpm = pd.read_excel(prop_filename, sheet_name=sheet_name)
            cls.rpm_sched_data__rpm = df_rpm['RPM'].values
            cls.rpm_sched_data__power_W = df_rpm['SHP'].values * 745.7

    @classmethod
    def load_config_data(cls, prop_filename, sheet_name='INFO'):
        if cls._config_data_loaded:
            return
        df_config = pd.read_excel(prop_filename, sheet_name=sheet_name)
        cls._KwmdDrag = df_config['KwmdDrag'].values[0]  # Get the first (and likely only) value
        cls._slipstream_data = df_config['slipStream'].values[0]
