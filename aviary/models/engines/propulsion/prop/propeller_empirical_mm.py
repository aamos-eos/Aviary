...
class EmpiricalPropellerCoeffMM(om.Group):
    ...
    def _prepare_grid_data(cls):
        ...
        J_grid, Cp_grid = np.meshgrid(vectJ, vectCp, indexing='ij')
        grid_points_Cp = np.column_stack([J_grid.flatten(), Cp_grid.flatten()])
        mapEta_flat_Cp = griddata(np.column_stack([J_data, Cp_data]), eta_data, grid_points_Cp, method='cubic', fill_value=0.3)
        mapEta_from_Cp = mapEta_flat_Cp.reshape(len(vectJ), len(vectCp))

        J_grid, Ct_grid = np.meshgrid(vectJ, vectCt, indexing='ij')
        grid_points_Ct = np.column_stack([J_grid.flatten(), Ct_grid.flatten()])
        mapEta_flat_Ct = griddata(np.column_stack([J_data, Ct_data]), eta_data, grid_points_Ct, method='cubic', fill_value=0.3)
        mapEta_from_Ct = mapEta_flat_Ct.reshape(len(vectJ), len(vectCt))
    ...
    def setup(self):
        ...
        if power_set:
            ...
            if not test_interp:
                ...
                self.add_subsystem('matrix_to_vector_cp', 
                                MatrixToVectorConverter(
                                    ...
                                    input_names=['adv_ratio', 'Cp'],
                                    units={'adv_ratio': None, 'Cp': None}
                                ), 
                                promotes_inputs=['adv_ratio', 'Cp'])
            ...
            Cp_eta_interp = om.MetaModelStructuredComp(vec_size=nn * npp, method='akima')
        ...
        if thrust_set:
            ...
            # Create regular grid interpolator instance with vectorization
            Ct_eta_interp = om.MetaModelStructuredComp(vec_size=nn * npp, method='akima')
...
def run_prop_model(test_interp = False, thrust_set = False, power_set = True, rpm_set = True,plot_results = False
    ...
    velocity_profile = np.linspace(125, 129.64, nn)
    ...
    num_engines = 4
    thrust_profile_unit = drag_N / num_engines
    ...
    model.add_subsystem('solve_propeller', EmpiricalPropellerCoeffMM(num_nodes=nn, 
                                                            num_props=npp,
                                                          thrust_set = thrust_set, 
                                                          power_set = power_set, 
    ...
    if plot_results:
        ...
        if test_interp:
            ...
            if thrust_set: 
                ...
                J_ct_interpolator_nearest = NearestNDInterpolator(np.column_stack([PropellerData.dyn_J_data, PropellerData.dyn_Ct_data]), PropellerData.dyn_eta_data)
                eta_nearest = J_ct_interpolator_nearest(j_val, ct_val)/ 100
                power_calc_nearest = thrust_calc_profile * np.tile(velocity_profile, npp) / eta_nearest / 1000
            elif power_set:
                cp_val = prob.get_val('Cp').flatten()
                J_cp_interpolator_nearest = NearestNDInterpolator(np.column_stack([PropellerData.dyn_J_data, PropellerData.dyn_Cp_data]), PropellerData.dyn_eta_data)
                eta_nearest = J_cp_interpolator_nearest(j_val, cp_val) / 100
                thrust_calc_nearest = eta_nearest[0:nn] * power_profile[0:nn] / velocity_profile
            # end
        # end
        ...
        # Plot 3: RPM profile
        plt.subplot(3, 3, 3)
        plt.plot(nodes, rpm_profile[0:nn], 'r-', linewidth=2, marker='o', label='Interpolated')
        plt.legend()
        plt.xlabel('Nodes')
        plt.ylabel('RPM')
        ...
        plt.subplot(3, 3, 4)
        plt.plot(nodes, power_profile[0:nn]/1000, 'purple', linewidth=2, marker='o', alpha=1.0, label='Metamodel')
        if test_interp and thrust_set:
            plt.plot(nodes, power_calc_nearest[0:nn]/1000, 'purple', linewidth=2, marker='o', alpha=0.2, label='Nearest Neighbour')
        plt.legend()
        if test_interp and thrust_set:
            for i in range(npp):
        ...
        thrust_calc_profile = prob.get_val('thrust_calc').flatten()
        plt.subplot(3, 3, 5)
        plt.plot(nodes, thrust_calc_profile[0:nn]/1000, 'orange', linewidth=2, marker='o', alpha=1.0, label='Metamodel')
        if test_interp and power_set:
            plt.plot(nodes, thrust_calc_nearest[0:nn]/1000, 'orange', linewidth=2, marker='o', alpha=0.2, label='Nearest Neighbour')
        plt.legend()
        if test_interp and power_set:
            for i in range(npp):
        ...
        plt.subplot(3, 3, 6)
        plt.plot(nodes, J_profile[0:nn], 'brown', linewidth=2, marker='o')
        ...
        plt.subplot(3, 3, 7)
        plt.plot(nodes, eta_profile[0:nn], 'cyan', linewidth=2, marker='o', label='Metamodel')
        if test_interp:
            plt.plot(nodes, eta_nearest[0:nn], 'cyan', linewidth=2, marker='o', alpha=0.2, label='Nearest Neighbour')
        plt.legend()
        plt.xlabel('Nodes')
...
def _get_propeller_interpolation_results():
    ...
    Uses EmpiricalPropellerCoeffMM twice: once for power_set and once for thrust_set.
    Returns: interpolated_values_Cp, interpolated_values_Ct, actual_values, relative_errors_Cp, relative_errors_Ct
    """
    # Get the data
    prop_data = PropellerData.get_data(prop_filename='aviary/models/engines/propulsion/empirical_data/DOWTY_prop_6blade_13ft.xlsx', sheet_name='data')
    ...
    # Reshape to (npp, nn) format expected by EmpiricalPropellerCoeffMM
    ...
    model_power.add_subsystem('prop', EmpiricalPropellerCoeffMM(
        num_nodes=nn, 
        num_props=npp, 
        power_set=True, 
        thrust_set=False, 
    ...
    model_thrust.add_subsystem('prop', EmpiricalPropellerCoeffMM(
        num_nodes=nn, 
        num_props=npp, 
        power_set=False, 
        thrust_set=True, 
...
if __name__ == "__main__":
    # Test interpolation accuracy using EmpiricalPropellerCoeffMM
    run_interpolation_accuracy_test(assert_test=False)
    
    # Test Propeller Model based on Cp/Ct for Thrust 
    run_prop_model(test_interp = True, thrust_set = False, power_set = True, rpm_set = True, plot_results = True)
    #om.n2(prob)

    # Test Propeller Interpolation out of bounds
    #eval_prop_coeff_bounds()
