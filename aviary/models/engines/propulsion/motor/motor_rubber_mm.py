...
def _get_motor_interpolation_results(plot_error=False):
    """
    Helper function to get motor interpolation results for testing using RubberMotor group
    Parameters:
    -----------
    ...
    # Set up the model using RubberMotor group
    model = om.Group()
    ivc = om.IndepVarComp()
    ...
    model.add_subsystem('motor', RubberMotor(
    ...
    # Test 3: Power/Voltage to Efficiency Interpolation using RubberMotor
    n_test_points_power_eff = min(20, len(MotorDataThrottleEffCurve.voltage_data))
    np.random.seed(456)
    test_indices_power_eff = np.random.choice(len(MotorDataThrottleEffCurve.voltage_data), n_test_points_power_eff, replace=False)
    
    model3 = om.Group()
    ivc3 = om.IndepVarComp()
    
    test_voltage_power_eff = MotorDataThrottleEffCurve.voltage_data[test_indices_power_eff]
    test_power_power_eff = MotorDataThrottleEffCurve.power_data__W[test_indices_power_eff] / 1000  # Convert to kW
    actual_eff_power_eff = MotorDataThrottleEffCurve.eff_data[test_indices_power_eff]
    ...
    model3.add_subsystem('motor', RubberMotor(
...
class RubberMotor(om.Group):
    ...
    def _load_data(cls):
        ...
        _pow_eff_volts_unique = np.unique(MotorDataThrottleEffCurve.voltage_data)
        _pow_eff_throttle_unique = np.unique(MotorDataThrottleEffCurve.throttle_data)
        _pow_eff_eff_unique = MotorDataThrottleEffCurve.eff_data

        cls._pow_eff_volts_unique =_pow_eff_volts_unique
        cls._pow_eff_throttle_unique = _pow_eff_throttle_unique
        cls._pow_eff_eff_unique = _pow_eff_eff_unique.reshape(len(_pow_eff_volts_unique),len(_pow_eff_throttle_unique)).transpose()
    ...
    def setup(self):
        ...
        self.add_subsystem('matrix_to_vector_pow_eff', 
                            MatrixToVectorConverter(
                                ...
                                input_names=['throttle', 'voltage'],
                                units={'throttle': None, 'voltage': 'V'}
                            ), 
                            promotes_inputs=['*'], promotes_outputs=['*'])

        # print("motor pow eff interp")
        # MetaModel for power/voltage efficiency interpolation
        motor_throttle_eff_interp = om.MetaModelStructuredComp(
            vec_size=num_nodes*num_motors, 
            method='akima'
        )
        motor_throttle_eff_interp.add_input('throttle_vect', 0.9, training_data=self._pow_eff_throttle_unique, units=None, shape=(num_motors*num_nodes,))
        motor_throttle_eff_interp.add_input('voltage_vect', 800, training_data=self._pow_eff_volts_unique, units="V", shape=(num_motors*num_nodes,))
        motor_throttle_eff_interp.add_output('eff_vect', 0.95, training_data=self._pow_eff_eff_unique, units=None, shape=(num_motors*num_nodes,))
        motor_throttle_eff_interp.options['extrapolate'] = True
        self.add_subsystem('motor_throttle_eff_interp', motor_throttle_eff_interp, promotes_inputs=['*'], promotes_outputs=['*'])

        # Convert vector output back to matrix
        self.add_subsystem('vector_to_matrix_throttle_eff', 
...
def test_motor_components():
    ...
    throttle_val = 0.5 * np.ones((num_motors, num_nodes))
    max_power_val = 2300e3 * np.ones((num_motors, num_nodes))
    
    ivc.add_output('voltage', voltage_val, units='V', desc='Motor voltage')

    # Add independent variables
    ivc.add_output('max_power', max_power_val, units='W', desc='Max power')
    ivc.add_output('throttle', throttle_val, desc='Throttle fraction')
    ivc.add_output('mech_power', throttle_val * max_power_val, units='W', desc='Commanded power')

    ivc.add_output('rpm', 1000 * np.ones((num_motors, num_nodes)), units='rpm', desc='Motor speed')

    model.add_subsystem('ivc', ivc, promotes=['*'])
    model.add_subsystem('motor', RubberMotor(num_nodes=num_nodes, 
    ...
    if test_interp:
        ...
        voltage_throttle_eff_interpolator_nearest = NearestNDInterpolator(
            np.column_stack([MotorDataThrottleEffCurve.voltage_data, MotorDataThrottleEffCurve.throttle_data]),
            MotorDataThrottleEffCurve.eff_data
        )
        ...
        throttle_flat = throttle_val.flatten()
        eff_nearest = voltage_throttle_eff_interpolator_nearest(voltage_flat, throttle_flat)
